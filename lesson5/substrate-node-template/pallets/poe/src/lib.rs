...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_claim(origin: OriginFor<T>, claim: BoundedVec<u8, T::MaxClaimLength>) -> DispatchResultWithPostInfo {
            ...
            Self::deposit_event(Event::ClaimCreated(sender, claim.clone()));
        ...
        #[pallet::call_index(2)]
        #[pallet::weight(0)]
        pub fn transfer_claim(origin: OriginFor<T>, claim: BoundedVec<u8, T::MaxClaimLength>, to: T::AccountId) -> DispatchResultWithPostInfo {
            let sender = ensure_signed(origin)?;
